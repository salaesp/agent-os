...
export function Costs() {
  ...
  const anthropic = providers.find((p) => p.provider === 'anthropic') || { cost: 0, month: 0 };
  const openrouterProv = providers.find((p) => p.provider === 'openrouter') || { cost: 0, month: 0 };
  ...
  return (
    <>
      <PageHead title="Costos" sub="OpenRouter vs Claude · hoy, este mes y últimos 30 días" />
      ...
      <div class="grid" style="margin-bottom:18px">
        <SplitCard title="Hoy" total={c.today.cost} anthropic={c.today.anthropic} openrouter={c.today.openrouter} />
        <SplitCard title="Este mes" total={c.monthTotal} anthropic={anthropic.month} openrouter={openrouterProv.month} />
      ...
      <div class="card" style="margin-bottom:16px">
        ...
        <div style="display:flex;align-items:flex-end;gap:4px;height:120px;margin-top:14px">
          {recent.map((d) => (
            <div key={d.day} style="flex:1;display:flex;flex-direction:column;align-items:center;gap:4px;height:100%;justify-content:flex-end"
              title={`${d.day}: $${d.cost.toFixed(2)} (claude $${(d.anthropic || 0).toFixed(2)} · openrouter $${(d.openrouter || 0).toFixed(2)})`}>
        ...
        <div class="wrap" style="margin-top:10px;font-size:11px">
          <span class="chip small"><i style="width:8px;height:8px;border-radius:2px;background:var(--accent);display:inline-block;margin-right:5px" />Claude</span>
      ...
      <SettingsCard settings={settings.data} onSave={() => { settings.reload(); reload(); }} />

      <button class="chip small" style="margin:16px 0" onClick={() => setDetail((v) => !v)}>
        {detail ? 'Ocultar detalle' : 'Ver detalle (por proveedor / por modelo)'}
      </button>
...
// Tarjeta con el total y el split OpenRouter/Claude debajo, para "hoy" y "este mes".
function SplitCard({ title, total, anthropic, openrouter }) {
  return (
    <div class="card stat">
      <span class="num">${Number(total || 0).toFixed(2)}</span>
      <span class="lbl">{title}</span>
      <div class="muted mono" style="font-size:10.5px;margin-top:6px">
        Claude ${Number(anthropic || 0).toFixed(2)} · OpenRouter ${Number(openrouter || 0).toFixed(2)}
      </div>
    </div>
  );
}

// Saldo prepago de OpenRouter. El aviso usa un umbral ABSOLUTO en USD (configurable
// abajo), no % contra el crédito histórico: ese % baja solo con cada recarga, aunque
// el saldo real esté estable — por eso antes el aviso siempre parecía peor de lo que era.
function OpenRouterCard({ or }) {
  if (!or || or.remaining == null) {
    if (or?.used_total != null) {
      return (
        <div class="card stat">
    ...
    return <div class="card stat"><span class="num">—</span><span class="lbl">OpenRouter</span></div>;
  }
  const threshold = or.lowBalanceThreshold || 10;
  const zone = threshold * 2;
  const barPct = Math.max(0, Math.min(100, Math.round((or.remaining / zone) * 100)));
  const cls = or.remaining <= threshold ? 'err' : or.remaining <= zone ? 'warn' : '';
  return (
    <div class="card">
      ...
      <div class="spread" style="margin-top:6px;align-items:baseline">
        ...
        <span class="muted mono" style="font-size:11px">aviso bajo ${threshold}</span>
      </div>
      <div class="bar" style="margin-top:8px"><i class={cls} style={`width:${barPct}%`} /></div>
...
function SettingsCard({ settings, onSave }) {
  const [budget, setBudget] = useState(settings?.monthly_budget_usd || '');
  const [orThreshold, setOrThreshold] = useState(settings?.openrouter_low_balance_usd || '10');
  const [busy, setBusy] = useState(false);
  const [saved, setSaved] = useState(false);
  useEffect(() => { setBudget(settings?.monthly_budget_usd || ''); }, [settings?.monthly_budget_usd]);
  useEffect(() => { setOrThreshold(settings?.openrouter_low_balance_usd || '10'); }, [settings?.openrouter_low_balance_usd]);

  const save = async () => {
    setBusy(true); setSaved(false);
    try {
      await post('/api/settings/set', { key: 'monthly_budget_usd', value: budget });
      await post('/api/settings/set', { key: 'openrouter_low_balance_usd', value: orThreshold });
      setSaved(true); onSave();
    } finally { setBusy(false); }
  };
  return (
    <div class="card" style="margin-bottom:16px">
      <h3>Presupuesto y avisos</h3>
      <div class="grid" style="grid-template-columns:1fr 1fr;gap:14px;margin-top:10px">
        <div>
          <div class="muted" style="font-size:12px;margin-bottom:6px">Presupuesto mensual (USD) — avisa al 80% y al 100%</div>
          <div class="search"><input type="number" placeholder="ej: 500" value={budget} onInput={(e) => setBudget(e.target.value)} /></div>
        </div>
        <div>
          <div class="muted" style="font-size:12px;margin-bottom:6px">OpenRouter: avisar cuando el saldo baje de (USD)</div>
          <div class="search"><input type="number" placeholder="ej: 10" value={orThreshold} onInput={(e) => setOrThreshold(e.target.value)} /></div>
        </div>
      </div>
      <button class="chip filter-chip on" style="margin-top:12px" disabled={busy} onClick={save}>{busy ? '…' : 'Guardar'}</button>
